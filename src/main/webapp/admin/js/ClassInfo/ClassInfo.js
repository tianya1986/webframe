var classInfo={
		serverUrl:'/admin/classInfo/',		
		fmtOperate:function(val, row, index){
			return '<a href="javascript:void(0);" onclick="classInfo.openUpdate('+index+')">修改</a>'+
			'&nbsp;&nbsp;<a href="javascript:void(0);" onclick="classInfo.del('+val+')">删除</a>';
		},
		openCreate:function(){
		...
		search:function(){
			...
			var extendjson=$("#extendjson").val();
			//var sysCode = $("#sysCode").combobox("getValue");
			$('#site-datagrid').datagrid("reload",{className:className,address:address,extendjson:extendjson});	
		},
