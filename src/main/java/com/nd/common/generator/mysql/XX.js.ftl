var ${bean.value?uncap_first}={
		serverUrl:'/admin/${bean.value?uncap_first}/',		
		fmtOperate:function(val, row, index){
			return '<a href="javascript:void(0);" onclick="${bean.value?uncap_first}.openUpdate('+index+')">修改</a>'+
			'&nbsp;&nbsp;<a href="javascript:void(0);" onclick="s${bean.value?uncap_first}.del('+val+')">删除</a>';
		},
		openCreate:function(){
		...
		save:function(){
			$('#createSiteForm').form('submit',{
			    url:'/admin/${bean.value?uncap_first}',
